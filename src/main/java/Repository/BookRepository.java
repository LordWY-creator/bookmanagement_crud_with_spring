...
public class BookRepository {
	...
	public List<BookBean> showAllBook() {
		...
		try {
			PreparedStatement ps = con.prepareStatement("SELECT * FROM books where status<>0");
	...
	public int insertBook(BookBean bean) {
		...
		Connection con = UserConnection.getConnection();
		try {
			PreparedStatement ps = con.prepareStatement("insert into books(title,author,price,quantity) values(?,?,?,?)");
	...
	public int updateBook(BookBean bean) {
		...
		Connection con = UserConnection.getConnection();
		try {
			PreparedStatement ps = con.prepareStatement("update books set title=? , author=? , price=? , quantity=? where id=?");
			ps.setString(1, bean.getTitle());
			ps.setString(2, bean.getAuthor());
			ps.setDouble(3, bean.getPrice());
			ps.setInt(4, bean.getQuantity());
...
public int deleteBook(int id) {
		...
		Connection con = UserConnection.getConnection();
		try {
			PreparedStatement ps = con.prepareStatement("update books set status=0 where id=?");
			ps.setInt(1,id);
...
public List<BookBean> showDeletedBooks() {
	...
	Connection con = UserConnection.getConnection();
	try {
		PreparedStatement ps = con.prepareStatement("update books set status=1 where id=?");
		ps.setInt(1,id);
